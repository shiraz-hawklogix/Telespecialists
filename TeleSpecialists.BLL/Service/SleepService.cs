...
namespace TeleSpecialists.BLL.Service
{
    public class SleepService : BaseService
    {
        ...
        public KeyValuePair<long, bool> AddSchedule(user_schedule_sleep model, bool isAllDay)
        {
            model.uss_time_from = ParseScheduleTime(model.uss_time_from_calc.Value.ToString("HH:mm"));
            model.uss_time_to = ParseScheduleTime(model.uss_time_to_calc.Value.ToString("HH:mm"));
            ...
            _unitOfWork.SleepRepository.Insert(model);
            _unitOfWork.Save();
            _unitOfWork.Commit();
            return new KeyValuePair<long, bool>(model.uss_key, true);
        }
        ...
        public List<user_schedule_sleep> GetAll(bool isGetAllRequeset, string userId, DateTime startDate, DateTime endDate, List<string> Physicians, string phy_type)
        {
        ...
        public IQueryable<user_schedule_sleep> GetSchedule(DateTime dateTime)
        {
            return _unitOfWork.SleepRepository
                              .Query()
                              .Where(m => dateTime >= m.uss_time_from_calc.Value)
                              .Where(m => dateTime <= m.uss_time_to_calc.Value)
                              ;
        }
        public IQueryable<user_schedule_sleep> GetScheduleById(long id)
        {
            return _unitOfWork.SleepRepository
                              .Query()
        ...
        {
            _unitOfWork.SleepRepository.Delete(id);
            _unitOfWork.Save();
            _unitOfWork.Commit();
            return true;
        }
        ...
        private IQueryable<user_schedule_sleep> GetFilteredSchedule(bool isGetAllRequeset, string userId)
        {
            var query = _unitOfWork.SleepRepository.Query().Where(m => m.AspNetUser.IsActive && m.AspNetUser.IsDeleted == false);
