...
SELECT Id,convert(varchar,@starttime,101) as [date],(convert(varchar,@starttime,101) + ' ( ' + convert(varchar(5),uss_time_from_calc,114) + ' - ' +  convert(varchar(5),uss_time_to_calc,114) + ' ) ') as CreatedDate   ,Physician, StatusName, DATEDIFF(SECOND, pDataDate, psl_created_date) as diff
into #tempUserPR
FROM    (
...
select Id,[date],Physician, CreatedDate,sum(Available) Available,sum(TPA) TPA,sum(StrokeAlert) StrokeAlert,
sum(Rounding) Rounding,sum(STATConsult) STATConsult,sum([Break]) [Break]
from #tempDailyFinal 
group by Id,Physician,CreatedDate,[date]
