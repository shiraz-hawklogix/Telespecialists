CREATE PROCEDURE [dbo].[usp_case_dashboard_bkp]   
...
IF OBJECT_ID(N'tempdb..#caseData') IS NOT NULL BEGIN DROP TABLE #caseData END
...
Select  cas_key,   
  ...
  dbo.FormatSeconds(cts_arrival_to_needle_sec) as ArrivalToNeedleTime,
 ...
 AND (  
  ...
  case when @SortDir <> 'DESC' then ''  
        when @SortType = 'ArrivalToNeedleTime' then cts_arrival_to_needle_sec  
        end DESC,    
      case  
        when @SortDir <> 'ASC' then ''  
        when @SortType = 'ArrivalToNeedleTime' then cts_arrival_to_needle_sec  
 ...
 into #temp FROM  (SELECT cas_key,  
     ...
     FROM  [dbo].[case]) AS A CROSS APPLY PhysicianInitials.nodes ('/M') AS Split(a) where cas_key IN (select cas_key from #caseData)
...
--select psl_phs_key,#temp1.UserInitial,psl_created_date
--into #tempfinal 
...
select cas.*,final.psl_status_color,final.psl_status_name,final.UserInitial
from
#caseData cas
left join #finalUserStatus final on cas.cas_key = final.cas_key 
END
