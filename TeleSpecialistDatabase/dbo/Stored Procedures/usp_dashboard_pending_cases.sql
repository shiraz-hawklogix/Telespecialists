CREATE PROCEDURE [dbo].[usp_dashboard_pending_cases]
@filter VARCHAR(50) = '',
...
,(DATEDIFF(dd, cas_created_date, @currentDate_EST) + 1)
  -(DATEDIFF(wk, cas_created_date, @currentDate_EST) * 2)
  -(CASE WHEN DATENAME(dw, cas_created_date) = 'Sunday' THEN 1 ELSE 0 END)
  -(CASE WHEN DATENAME(dw, @currentDate_EST) = 'Saturday' THEN 1 ELSE 0 END) AS arivaltoneedle45
,cas_created_date
...
AND cas_is_active = 1 
AND cas_cst_key = 20 
AND cas_response_case_qps_reviewed is null
ORDER BY cas_billing_date_of_consult DESC

select cas_key AS CaseKey,cas_case_number AS TC_CaseNumber,FacilityName,QPS_Name,billing_date_of_consult AS DateOfConsult,case when arivaltoneedle45 > 10 then 'true' else 'false' end as ColorRed
