...
-- ============================================= 
CREATE PROCEDURE [dbo].[usp_dispatch_listing] 
...
Select  cas_key, 
        ...
        cas_cart, 
		CONCAT(Physician.FirstName, ' ' , Physician.LastName )  as phy_name,
        cas_cst_key, 
        cas_case_number,   
		...
		cas_patient,
		cas_phy_key,
		[dbo].[facility].fac_timezone AS FacilityTimeZone,
		cas_notes,
		cas_triage_notes,
		cas_callback_extension,
		cas_eta,
		--cahWaiting.waitingToAccept As case_assign_history_waitingToAccept,
		--cahAccept.accepted As case_assign_history_accepted,

		CASE WHEN @Take is not null THEN count(*) over() ELSE 0 END  as totalRecords

     
        FROM     [dbo].[case] AS ctbl (NOLOCK) 
		--OUTER APPLY 
		--(Select Top 1 ch.cah_created_date as waitingToAccept from [dbo].[case] c
		--Join [dbo].[case_assign_history] ch on ch.cah_cas_key = ctbl.cas_key
		--where
		--ch.cah_action = 'Waiting To Accept'
		--Order by ch.cah_key desc) cahWaiting
		--OUTER APPLY 
		--(Select Top 1 ch.cah_created_date as accepted from [dbo].[case] c
		--Join [dbo].[case_assign_history] ch on ch.cah_cas_key = ctbl.cas_key
		--where
		--ch.cah_action = 'Accepted'
		--Order by ch.cah_key desc) cahAccept
		...
		--INNER JOIN [dbo].[case_assign_history] cah  (NOLOCK)  ON cas_key = cah.cah_cas_key 
