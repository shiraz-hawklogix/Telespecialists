...
-- Post Stat Workshop 
					...
					IF exists (Select COUNT(*) from #physiciansQuery where AspNetUser_physician_status = 20 AND scheduleExist = 1)	-- Post Stat Workshop  Check
		 
					Select *
					INTO #SCHEDULED_For_PostSTAT_physicians			-- Scheduled Temp Table
					from #physiciansQuery where scheduleExist = 1 

						IF exists (Select COUNT(*) from #SCHEDULED_For_PostSTAT_physicians)

							Select * 
							INTO #PostSTATphysicians    -- Post Stat Workshop Temp Table
							From #SCHEDULED_For_PostSTAT_physicians where AspNetUser_physician_status = 20 AND scheduleExist = 1

								IF exists (Select COUNT(*) from #PostSTATphysicians) 

									INSERT INTO #FinalPhysicianTable
									SELECT PSTP.*, FinalSorted = 1
									FROM #PostSTATphysicians PSTP
									Order By AspNetUser_status_change_date ASC
				
							Drop table #PostSTATphysicians

					Drop table #SCHEDULED_For_PostSTAT_physicians
 
 
-- Post SA 

			IF exists (Select COUNT(*) from #physiciansQuery where AspNetUser_physician_status = 18 AND scheduleExist = 1)	-- Post SA Check
		 
					Select *
					INTO #SCHEDULED_For_POSTSA_physicians			-- Scheduled Temp Table
					from #physiciansQuery where scheduleExist = 1 

						IF exists (Select COUNT(*) from #SCHEDULED_For_POSTSA_physicians)

							Select * 
							INTO #PostSAphysicians    -- Post SA Temp Table
							From #SCHEDULED_For_POSTSA_physicians where AspNetUser_physician_status = 18 AND scheduleExist = 1

								IF exists (Select COUNT(*) from #PostSAphysicians) 

									INSERT INTO #FinalPhysicianTable
									SELECT PSA.*, FinalSorted = 1
									FROM #PostSAphysicians PSA
									Order By AspNetUser_status_change_date ASC
				
							Drop table #PostSAphysicians

					Drop table #SCHEDULED_For_POSTSA_physicians
...
-- Rounding Prep
			...
			IF exists (Select COUNT(*) from #physiciansQuery where AspNetUser_physician_status = 19 AND scheduleExist = 1)	-- Rounding Prep Check
		 
					Select *
					INTO #SCHEDULED_For_Rounding_Prep_physicians			-- Scheduled Temp Table
					from #physiciansQuery where scheduleExist = 1 

						IF exists (Select COUNT(*) from #SCHEDULED_For_Rounding_Prep_physicians)

							Select * 
							INTO #RoundingPrepphysicians    -- Rounding Prep Temp Table
							From #SCHEDULED_For_Rounding_Prep_physicians where AspNetUser_physician_status = 19 AND scheduleExist = 1

								IF exists (Select COUNT(*) from #RoundingPrepphysicians) 

									INSERT INTO #FinalPhysicianTable
									SELECT RPP.*, FinalSorted = 1
									FROM #RoundingPrepphysicians RPP
									Order By AspNetUser_status_change_date DESC --AspNetUser_CredentialIndex ASC , 
				
							Drop table #RoundingPrepphysicians

					Drop table #SCHEDULED_For_Rounding_Prep_physicians
...
-- Stat Consult
			...
			IF exists (Select COUNT(*) from #physiciansQuery where AspNetUser_physician_status = 8 AND scheduleExist = 1)	-- Stat Consult Check
		 
					Select *
					INTO #SCHEDULED_For_STAT_physicians			-- Scheduled Temp Table
					from #physiciansQuery where scheduleExist = 1 

						IF exists (Select COUNT(*) from #SCHEDULED_For_STAT_physicians)

							Select * 
							INTO #STATphysicians    -- Stat Consult Temp Table
							From #SCHEDULED_For_STAT_physicians where AspNetUser_physician_status = 8 AND scheduleExist = 1

								IF exists (Select COUNT(*) from #STATphysicians) 

									INSERT INTO #FinalPhysicianTable
									SELECT STP.*, FinalSorted = 1
									FROM #STATphysicians STP
									Order By AspNetUser_status_change_date ASC
				
							Drop table #STATphysicians

					Drop table #SCHEDULED_For_STAT_physicians
...
-- Stroke Alert || TPA 
			...
			IF exists (Select COUNT(*) from #physiciansQuery where AspNetUser_physician_status = 3 OR AspNetUser_physician_status = 2 AND scheduleExist = 1)	-- Stroke Alert || TPA   Check
					...
					from #physiciansQuery where scheduleExist = 1 
						...
						IF exists (Select COUNT(*) from #SCHEDULED_physicians)
							...
							From #SCHEDULED_physicians where AspNetUser_physician_status = 3 OR AspNetUser_physician_status = 2 AND scheduleExist = 1
...
--	BREAK
			...
			IF exists (Select COUNT(*) from #physiciansQuery where  AspNetUser_physician_status = 16  AND scheduleExist = 1)	--	BREAK  Check
		 
					Select *
					INTO #SCHEDULED_BREAK_physicians			-- Scheduled BREAK Temp Table
					from #physiciansQuery where scheduleExist = 1 

						IF exists (Select COUNT(*) from #SCHEDULED_BREAK_physicians)

							Select * 
							INTO #Breakphysicians    -- Break Temp Table
							From #SCHEDULED_BREAK_physicians where AspNetUser_physician_status = 16 AND scheduleExist = 1


					Drop Table #SCHEDULED_BREAK_physicians
					 
			--	BEGIN
					IF exists (Select COUNT(*) from #Breakphysicians) 

						INSERT INTO #FinalPhysicianTable
						SELECT BRP.*, FinalSorted = 1 
						FROM #Breakphysicians BRP
						Order By AspNetUser_status_change_date ASC
--				END

			Drop table #Breakphysicians
