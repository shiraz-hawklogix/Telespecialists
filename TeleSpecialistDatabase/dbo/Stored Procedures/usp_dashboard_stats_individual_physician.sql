CREATE PROCEDURE [dbo].[usp_dashboard_stats_individual_physician]
	@filter VARCHAR(50) = '',
...
BEGIN
	...
	set @DoorToNeedleTime = ISNULL((SELECT CONVERT(VARCHAR, ( CAST(CAST(AVG(CAST((cas_metric_needle_time - cas_metric_door_time) AS FLOAT)) AS DATETIME) AS TIME(0)) ))
										FROM [case] (NOLOCK) 
										WHERE cas_created_date >= @startDate AND cas_metric_tpa_consult = 1 AND cas_metric_wakeup_stroke = 0 AND cas_phy_key = @physician),'00:00:00')
	...
	set @LogonToNeedleTime = ISNULL((SELECT CONVERT(VARCHAR, ( CAST(CAST(AVG(CAST((cas_metric_needle_time - cas_response_first_atempt) AS FLOAT)) AS DATETIME) AS TIME(0)) ))
										FROM [case] (NOLOCK) 
										WHERE cas_created_date >= @startDate AND cas_phy_key = @physician),'00:00:00')
	...
	SELECT 
	TotalCases = (SELECT COUNT(*)  FROM [case] (NOLOCK) WHERE cas_created_date >= @startDate AND cas_phy_key = @physician),
	TotalStrokeAlert = (SELECT COUNT(*)  FROM [case] (NOLOCK) cas_ctp_key WHERE cas_ctp_key = 9 AND cas_created_date >= @startDate AND cas_phy_key = @physician),
	TotalStatConsults = (SELECT COUNT(*)  FROM [case] (NOLOCK) cas_ctp_key WHERE cas_ctp_key = 10 AND cas_created_date >= @startDate AND cas_phy_key = @physician),
	...
	set @DoorToNeedleTime = ISNULL((SELECT CONVERT(VARCHAR, ( CAST(CAST(AVG(CAST((cas_metric_needle_time - cas_metric_door_time) AS FLOAT)) AS DATETIME) AS TIME(0)) ))
										FROM [case] (NOLOCK) 
										WHERE cas_created_date >= @startDateTime AND cas_metric_tpa_consult = 1 AND cas_metric_wakeup_stroke = 0 AND cas_phy_key = @physician),'00:00:00')

	set @LogonToNeedleTime = ISNULL((SELECT CONVERT(VARCHAR, ( CAST(CAST(AVG(CAST((cas_metric_needle_time - cas_response_first_atempt) AS FLOAT)) AS DATETIME) AS TIME(0)) ))
										FROM [case] (NOLOCK) 
										WHERE cas_created_date >= @startDateTime AND cas_phy_key = @physician),'00:00:00')
	...
	SELECT 
	TotalCases = (SELECT COUNT(*)  FROM [case] (NOLOCK) WHERE cas_created_date >= @startDateTime AND cas_phy_key = @physician),
	TotalStrokeAlert = (SELECT COUNT(*)  FROM [case] (NOLOCK) cas_ctp_key WHERE cas_ctp_key = 9 AND cas_created_date >= @startDateTime AND cas_phy_key = @physician),
	TotalStatConsults = (SELECT COUNT(*)  FROM [case] (NOLOCK) cas_ctp_key WHERE cas_ctp_key = 10 AND cas_created_date >= @startDateTime AND cas_phy_key = @physician),
